...
import { Send, MessageSquare, Move, Pin } from 'lucide-react';
...
const JobNotesSection: React.FC<JobNotesSectionProps> = ({ job }) => {
    ...
    useEffect(() => {
        ...
        // Short Polling for Live Chat feel
    ...
    return (
        <div className="flex flex-col h-full overflow-hidden">
            ...
            <div 
                ref={scrollRef}
                className="flex-grow overflow-y-auto p-4 space-y-4 bg-surface-container scrollbar-thin scrollbar-thumb-surface-container-highest"
            >
                {isLoading ? (
                        ...
                        return (
                            <div key={note.id} className={`flex gap-3 group ${isMe ? 'flex-row-reverse' : ''}`}>
                                ...
                                <div className="flex-shrink-0 mt-1">
                                    {note.authorAvatar ? (
                                        <img src={note.authorAvatar} alt={note.authorName} className="w-8 h-8 rounded-full object-cover border-2 border-surface-container-high" />
                                    ) : (
                                        <>
                                            <img 
                                            ...
                                            />
                                            <div className="hidden w-8 h-8 rounded-full bg-primary-container text-on-primary-container flex items-center justify-center text-sm font-bold border-2 border-surface-container-high">
                                ...
                                <div className={`flex flex-col max-w-[85%] ${isMe ? 'items-end' : 'items-start'}`}>
                                    <div className={`flex items-baseline gap-2 mb-1 ${isMe ? 'flex-row-reverse' : ''}`}>
                                        ...
                                        <button 
                                            ...
                                            className={`transition-opacity ${note.isPinned ? 'opacity-100 text-tertiary' : 'opacity-0 group-hover:opacity-100 text-text-tertiary hover:text-text-primary'}`}
                                    ...
                                    <div className={`p-3 rounded-xl text-sm whitespace-pre-wrap shadow-sm relative ${
                                        note.isPinned ? 'ring-2 ring-tertiary/80' : ''
                                    } ${
                                        isMe 
            ...
            {/* Input Area */}
            <div className="p-3 bg-surface-container-low border-t border-outline/10 flex-shrink-0">
                <form onSubmit={handleSendNote} className="flex items-end gap-2">
                    ...
                    <button 
                        ...
                        className="flex-shrink-0 h-[42px] w-[42px] flex items-center justify-center bg-primary hover:bg-primary-hover text-on-primary rounded-full disabled:opacity-50 disabled:cursor-not-allowed transition-colors shadow-sm"
                    >
                        <Send size={18} />
                    </button>
                </form>
